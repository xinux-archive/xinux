...
# keyring.sh
#
# Script to import Xinux and ArchLinux keys.
#
...
update_alter_key() {
    ...
    pacman --config "${alter_pacman_conf_x86_64}" -Sy --noconfirm xinux/xinux-keyring
...
remove_alter_key() {
    ...
    if checkpkg xinux-keyring; then
        pacman -Rsnc --noconfirm xinux-keyring
    fi
}
...
# Hamma narsani argumentsiz ishga tushuradi
if [[ ${#} = 0 ]]; then
    run prepare
    # run update_arch_key
    run update_alter_key
