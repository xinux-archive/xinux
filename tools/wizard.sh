...
:<< TEXT
Funksiya va Joylarga Ot qo'yish qoidalari

<< Global >>
Global Joy: Var_Global_[Joy Oti]
Global Funksiya: Function_Global_[Funksiya Oti]

[ Faqatgina Function_Global_Main_ask_questions ichida ishlatilgan funksiyalar ]
Global Funksiya: Function_Global_Ask_[qo'yilishi kerak bo'lgan joy oti]

[ wizard.sh uchun joylar ]
Tanlovlar uchun: Var_Global_Wizard_Option_
Env sozlamalari: Var_Global_Wizard_Env_

<< Lokal >>
Lokal Joy: Var_Local_[Joy Oti]
Lokal Funksiya: Var_Local_[Funksiya Oti]
TEXT
...
# Til（en or uz)
#Var_Global_Wizard_Option_language="uz"
Var_Global_Wizard_Option_language="en"
Var_Global_Wizard_Option_skip_language=false



# Biron habar ko'rsatish
# msg [O'zbek] [Ingliz]
msg() {
    if [[ ${Var_Global_Wizard_Option_language} = "uz" ]]; then
        echo "${1}"
    else
        echo "${2}"
    fi
...
# Usage: getclm <number>
# Standard kirimdan qiymatni olib turib argumentlarni chiqarish
getclm() { cut -d " " -f "${1}"; }
...
while getopts 'a:xnjeh' arg; do
    case "${arg}" in
        n)
            ...
            msg \
                "Stimulyatsiya holatini ishga tushurish" "Enabled simulation mode"
            ;;
        x)
            set -x
            msg "Debugging holatini ishga tushurish" "Debug mode enabled"
            ;;
        e)
            Var_Global_Wizard_Option_language="en"
            echo "English is set"
...
Function_Global_Main_wizard_language () {
    if [[ "${Var_Global_Wizard_Option_skip_language}" = false ]]; then
        echo "Ushbu yo'llanmada qaysi tilni ishlatishni xohlaysiz?"
        echo "Bu savol faqat ushbu yo'llanmada ishlatiladi va qurilish qismiga ta'siri yo'q"
        echo
        ...
        case "${Var_Global_Wizard_Option_language}" in
            "1" | "Ingliz"   | "English"  | "en") Var_Global_Wizard_Option_language=en ;;
            "2" | "O'zbek" | "Uzbek" | "uz") Var_Global_Wizard_Option_language=uz ;;
            *                                ) Function_Global_Main_wizard_language ;;
        esac
    fi
}
...
Function_Global_Main_check_required_files () {
    ...
    for Var_Local_file in "${Var_Local_file_list[@]}"; do
        if [[ ! -f "${Var_Global_Wizard_Env_script_path}/${Var_Local_file}" ]]; then
            msg_error "${Var_Local_file} topilmadi." "${Var_Local_file} was not found."
...
Function_Global_Main_check_yay(){
    if ! pacman -Qq yay > /dev/null 2>&1; then
        msg_error "yay topilmadi." "yay was not found."
        exit 1
    fi
}
...
Function_Global_Main_install_dependent_packages () {
    ...
    msg "Paketlar bazasi yangilanayabdi..." "Updating package datebase..."
...
# Yig'ish sozlamlarini tekshirish 
Function_Global_Ask_Confirm () {
    msg "Ushbu sozlamlar bilan yig'ishni boshlang." "Start the build with the following settings."
    echo
    #[[ -n "${Var_Global_Build_uzbek}"              ]] && echo "              Uzbek : ${Var_Global_Build_japanese}"
    ...
    echo
    msg_n \
        "Ushbu sozlam bilan davom eting。Hammasi sozmi？ (y/N) : " \
...
Function_Global_Main_ask_questions () {
    ...
    Function_Global_Ask_username
    Function_Global_Ask_password
    # Function_Global_Ask_uzbek Ushbu funksiya Xinux02 va undan ertalariga moslangan.
    Function_Global_Ask_locale
    Function_Global_Ask_channel
    Function_Global_Ask_tarball

    # Shu joylar sohibi o'zgaradi va fayllar keyinchalik taqiqlanadi.
    # Function_Global_Ask_owner
    # Function_Global_Ask_out_dir
